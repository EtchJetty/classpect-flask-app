...
def dualName(unknown):  # compatibility function
    if unknown.__class__ != [].__class__:
        if unknown.__class__ == ClasspectComponent("base", "class").__class__:
            if unknown.isCanon():
                return "<a href='#collapseOne' class='link-primary text-decoration-none' onclick='copycat(\"" + unknown.name + "\");' role='button'>" + unknown.name + "</a>"
        return unknown.name
    else:
        x = ""
        for i in unknown:
...
def magicant(form, results, formState):
    for i in results:
        megaarr = (
            ...
            (
                ClasspectComponent(
                    results[i].name, results[i].typeInverse()).isCanon()
                or
                ClasspectComponent(
                    results[i].name, results[i].typeInverse()).isDual()
            )
            and
            (
                ...
                (
                    ...
                    (
                        (invalidCspect(
                            results["math"+results[i].typeInverse()]))
                        and
        ...
        try:
            ...
            if last[2] == last[3]:
                if not invalidCspect(ClasspectComponent(last[2], "class")):
                    validtype = "aspect"
                    results["math" +
                            validtype] = ClasspectComponent("", validtype)
...
def dualFlavorText(formState, classpect_data):
    ...
    # makes list, stringifies, puts the small tag + class around the string
    printable_dual_classpect_flavor_text = wrapMuted(
        "".join(makePrintable(formState, dual_classpect_data)))
...
def mathValidator(form):
    ...
    results = {}
    for i in form:
        results[i] = (ClasspectComponent(name=form[i].capitalize(),
                      type=i.capitalize().replace("Math", "")))
...
def searchpage():
    if len(request.args) > 0:
        ...
        if "".join([value for key, value in argargs.items()]) != "":
            request.method = "POST"
            hidden = {"class": argargs["c"], "aspect": argargs["a"],
    ...
    if request.method == "POST":
        ...
        for i in form:
            results[i] = ClasspectComponent(
                name=form[i].capitalize(), type=i.capitalize().replace("Math", ""))
            if results[i].isDual():
                formState["dual"] = True
        ...
        if formState["dual"]:
            printable_dual_classpect_flavor_text = "<br>" + \
                dualFlavorText(formState, classpect_data)
        elif class_data.isCanon() and aspect_data.isCanon():
            printable_dual_classpect_flavor_text = "<br><small class='text-muted'>Examples include: " + \
                CLASSPECT_DICT[class_data.name][aspect_data.name] + "</small>"
        else:
        ...
        if not invalidCspects(classpect_data):
            ...
            printable_inverse_classpect = wrapLi(
                "".join(makePrintable(formState, inverse_classpect_data)), "Inverse")
            if formState["dual"]:
                printable_inverse_dual_classpect_flavor_text = "<ul style='margin-bottom: 0px'><li style='color: transparent'>" + \
                    dualFlavorText(
                        formState, inverse_classpect_data) + "</li></ul>"
            else:
    ...
    <div id="collapseOne" class="collapse open" aria-labelledby="headingOne">
      <div class="card-body" style="padding: unset;">
        ...
        if not invalidCspect(class_data):
            printable_paired_class = wrapLi(dualName(class_data.paired(
            )), "Paired class for " + dualName(class_data))  # future me i am so sorry
            if formState["dual"]:
                printable_paired_dual_class_flavor_text = "<ul style='margin-bottom: 0px'><li style='color: transparent'>" + \
                    dualFlavorText(formState, (class_data.paired(
                    ), ClasspectComponent("", "aspect"))) + "</li></ul>"
            else:
        ...
        display = [printable_classpect, printable_dual_classpect_flavor_text, hr1, printable_inverse_classpect,
                   printable_inverse_dual_classpect_flavor_text, printable_paired_class, printable_paired_dual_class_flavor_text, hr2]
        ...
        if mathValidator(form):
            ...
            tooltip = [
                "Add or subtract classpects to make Dual Classes!<br><small>It looks like you've searched for a "]
            if not formState["dual"] and (class_data.isCanon() or aspect_data.isCanon()):
                tooltip.extend(
                    "standard Classpect. This means you can add any other standard classpect to it, and it'll give you a unique dual!</small>")
                ...
                validmath = {
                    "class": components["class"], "aspect": components["aspect"]}
                if components["class"] == []:
                ...
                if not (class_data.isCanon() or aspect_data.isCanon()):
                    tooltip.extend("Dual Classpect. Since the Dual Classpect is made up of regular Classpects, you can subtract any of its components <span class='text-muted'>(like " +
                                   ex1 + " or " + ex2 + ")</span> from your search term!</small>")
                    ...
                    tooltip.extend("mix of the two. Since the Dual Classpect is made up of regular Classpects, you can subtract any of its components <span class='text-muted'>(like " + ex1 + " or " + ex2 +
                                   ")</span> from your search term, but you can also add any regular classpect <span class='text-muted'>(like " + ex3 + " or " + ex4 + ")</span> to the non-Dual part of your search!</small>")
        ...
        if formState["math"]:
            ...
            if mathValidator(form):
                # math tooltip generation
                if not formState["dual"] and (class_data.isCanon() or aspect_data.isCanon()):
                    summed_data = results["class"] + \
                        results["mathclass"], results["aspect"] + \
                        results["mathaspect"]
                    if (summed_data[0].name + summed_data[1].name) != "":
                        printable_sum = wrapLi(
                            "".join(makePrintable(formState, summed_data)), "Sum")
                        addit = "<ul style='margin-bottom: 0px'><li style='color: transparent'>" + \
                            dualFlavorText(
                                formState, summed_data) + "</li></ul>"
                    ...
                    if not (class_data.isCanon() or aspect_data.isCanon()):
                        ...
                        if (summed_data[0].name + summed_data[1].name) != "":
                            printable_sum = wrapLi(
                                "".join(makePrintable(formState, summed_data)), "Difference")
                        ...
                        if (summed_data[0].name + summed_data[1].name) != "":
                            printable_sum = wrapLi(
                                "".join(makePrintable(formState, summed_data)), "Output")
                            addit = "<ul style='margin-bottom: 0px'><li style='color: transparent'>" + \
                                dualFlavorText(
                                    formState, summed_data) + "</li></ul>"
                            mathdisplay.extend([printable_sum, addit])
                ...
                for i in classpect_data:
                    if invalidCspect(i):
                        if (form[i.typeInverse()] == "") and not invalidCspect(ClasspectComponent(form["math" + i.typeInverse()], i.typeInverse())):
                            summed_data = sorted((i, ClasspectComponent(
                                form["math" + i.typeInverse()], i.typeInverse())), key=sortByType, reverse=True)
                            printable_sum = wrapLi(
                                "".join(makePrintable(formState, summed_data)), "Output")
                            addit = "<ul style='margin-bottom: 0px'><li style='color: transparent'>" + \
                                dualFlavorText(
...
def homepageget():
    ...
    listy = [[{inx["type"]:ClasspectComponent(inx["name"], inx["type"]).__dict__}
              for inx in types] for types in dict(fetchAllClasspects()).values()]
    listy = [{"class": i["class"], "aspect":y["aspect"]}
             for i in listy[0] for y in listy[1]]
    random.Random(413).shuffle(listy)
    listy = json.dumps(dict([(str(indexy), itemy)
                       for indexy, itemy in enumerate(listy)]))
    # random.seed(413)
    # seeder = random.getstate()
    # random.Random(413).shuffle(listy)
    dual_listy = fetchAllDuals()
    dual_listy = [{"class": i, "aspect": y} for i in sorted(
        dual_listy["class"]) for y in sorted(dual_listy["aspect"])]
    ...
    print(daysNum, "days")
    smalldual_listy = [{"class": dual_listy[i]["class"], "aspect":dual_listy[i]["aspect"], "aspectduals":[component.__dict__ for component in ClasspectComponent(
        dual_listy[i]["aspect"], "aspect").dualComponents()]} for i in range((abs(daysNum) - 3), (abs(daysNum) + 3))]
...
def testsearchpage():
    ...
    for fixe in ["c", "a", "mc", "ma"]:
        try:
            ...
            argargs[fixe] = ""
    # if "".join([value for value in argargs.values()]) != "":
        # request.method = "POST"
